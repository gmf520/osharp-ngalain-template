...
import { STColumnTag, STRequestOptions, STReq, STData, STRes, STPage } from '@delon/abc/st';
...
export class AlainService {
  ...
  ToSFSchema(column: OsharpSTColumn): SFSchema {
    ...
    for (const key of keys) {
      if (schemaProps.includes(key)) {
        schema[key] = column.properties![key];
      } else if (specialProps.includes(key)) {
        // fkey的情况，直接使用fkey的值
        let ekey = key.substr(1);
        schema[ekey] = column.properties![key];
  ...
  changeFilterRuleType(rule: FilterRule, column: OsharpSTColumn) {
    ...
    switch (type) {
      ...
      case 'number':
        if (column.enum || column.tag) {
          rule['entries'] = this.osharp.getOperateEntries([FilterOperate.Equal, FilterOperate.NotEqual]);
